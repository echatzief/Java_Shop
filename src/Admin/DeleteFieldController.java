 ...
 * @author Apostolis
 */
public class DeleteFieldController implements Initializable {
    ...
    @FXML
    private void deleteFieldFromTable(MouseEvent event) throws SQLException {
        ...
        if(tName.length()!=0 && fName.length()!=0 ){
            ...
            if(selectedTable.next()){
                if(tName.equals("User") || tName.equals("Order")){
                    queryStr = String.format("ALTER TABLE \""+tName+"\" DROP COLUMN "+fName+" CASCADE");
                }
                else{
                    queryStr = String.format("ALTER TABLE "+tName+" DROP COLUMN "+fName+" CASCADE");
                }
    ...
    @FXML
    private void deleteFieldFromTableWithKey(KeyEvent event) throws SQLException {
